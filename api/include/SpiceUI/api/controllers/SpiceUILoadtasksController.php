...
class SpiceUILoadtasksController
{
    ...
    /**
     * execute load task item by name and return the class method result
     * @param Request $req
     * @param Response $res
     * @param array $args
     * @return Response
     * @throws \Exception
     */
    public function executeLoadTaskItemByName(Request $req, Response $res, array $args): Response {
        $db = DBManagerFactory::getInstance();
        $responseArray = [];
        $taskitems = $db->query("SELECT * FROM sysuiloadtaskitems WHERE name = '{$args['name']}' UNION SELECT * FROM sysuicustomloadtaskitems WHERE name = '{$args['name']}'");
        while ($taskitem = $db->fetchByAssoc($taskitems)) {
            // check if static call or not
            try {
                if (strpos($taskitem['method'], '::') > 0) {
