...
class LogViewer {
    ...
    public function getEntries( $queryParams ) {
        ...
        if ( !empty( $queryParams['pinned'])) $whereClauseParts[] = "l.pinned = 1";
        ...
        switch($db->dbType) {
            case 'oci8':
                $sql = 'SELECT u.id as user_id, l.date_entered, l.id, l.pid, l.log_level, l.log_sublevel, l.transaction_id, pinned, SUBSTR( l.description, 1, ' . $this->maxLength . ' ) AS description, l.created_by, u.user_name, CASE WHEN  LENGTH(l.description) > 500 THEN 1 ELSE 0 END descriptionTruncated FROM syslogs l LEFT JOIN users u ON u.id = l.created_by ' . $whereClause . ' ORDER BY l.date_entered DESC';
                break;
            default:
                $sql = 'SELECT u.id as user_id, l.date_entered, l.id, l.pid, l.log_level, l.log_sublevel, l.transaction_id, pinned, SUBSTR( l.description, 1, ' . $this->maxLength . ' ) AS description, l.created_by, u.user_name, if ( LENGTH( l.description ) <> LENGTH( SUBSTR( l.description, 1, ' . $this->maxLength . ' )), 1, 0 ) AS descriptionTruncated FROM syslogs l LEFT JOIN users u ON u.id = l.created_by ' . $whereClause . ' ORDER BY l.date_entered DESC';
                break;
        }
