...
class DBManagerFactory
{
    ...
    public static function getInstance(string $instanceName = 'initial')
    {
        ...
        if (!isset(self::$instances[self::$instanceName])) {
            ...
            self::$dbConfig = SpiceConfig::getInstance()->config['dbconfig'];

            $count++;
            self::$instances[self::$instanceName] = self::getTypeInstance(self::$dbConfig['db_type'], self::$dbConfig);
            ...
            self::$instances[self::$instanceName]->connect(self::$dbConfig, true);
    ...
    public static function switchDatabase($dbName, string $instanceName = 'initial')
    {
        ...
        self::$dbConfig['dbconfig']['db_name'] = $dbName;

        self::$instances[$instanceName] = self::getTypeInstance(self::$dbConfig['dbconfig']['db_type'], self::$dbConfig['dbconfig']);
        if (!empty(self::$dbConfig['dbconfigoption'])) {
            self::$instances[$instanceName]->setOptions(self::$dbConfig['dbconfigoption']);
        }
        self::$instances[$instanceName]->connect(self::$dbConfig['dbconfig'], true);
