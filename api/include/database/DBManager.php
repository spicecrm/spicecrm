...
abstract class DBManager
{
    ...
    {
        LoggerManager::getLogger()->deprecated('Call to DBManager::$' . $p . ' is deprecated');
        return $this->$p;
    }
    ...
    protected function registerError($userMessage, $message, $dieOnError = false)
    {
        if (!empty($message)) {
            ...
            LoggerManager::getLogger()->fatal('sqlerror',$message);
        }
    }
    ...
    protected function dump_slow_queries($query)
    {
        ...
        $slow_query_time_msec = isset(SpiceConfig::getInstance()->config['slow_query_time_msec']) ? SpiceConfig::getInstance()->config['slow_query_time_msec'] : 5000;


        if ($slow_query_time_msec < ($this->query_time * 1000)) {
            // Then log both the query and the query time
            LoggerManager::getLogger()->slowsql('', ['Query Execution Time' => $this->query_time, "Query" => $query]);
            return true;
        }
...
//			}
            ...
            if (strncasecmp($qstart, $query, strlen($qstart)) == 0) {
                if (is_callable([$this, $check])) {
                    ...
                    if (!in_array($table, $skipTables)) {
                        ...
                        LoggerManager::getLogger()->debug('sql', "Skipping table $table as blacklisted");
                    }
                } else {
                    LoggerManager::getLogger()->debug('sql', "No verification for $qstart on {$this->dbType}");
                }
                break;
            }
        }
    ...
    {
        LoggerManager::getLogger()->debug('sql', 'verifying CREATE statement...');

        // rewrite DDL with _temp name
        LoggerManager::getLogger()->debug('sql', 'testing query: [' . $query . ']');
        ...
        LoggerManager::getLogger()->debug('sql', 'testing for table: ' . $table);
    ...
    {
        LoggerManager::getLogger()->debug('sql', "DBManager.commit() stub");
        return true;
    }
    ...
    {
        LoggerManager::getLogger()->debug('sql', "DBManager.rollback() stub");
        return false;
    }
