    ...
    static function getBeanIndexProperties($module, $overrideCache = false)
    {
        ...
        if (!$overrideCache && $cached && isset($cached[$module])) {
            ...
            if ($moduleProperties) {
                ...
                if ($seed && isset($seed->field_defs['is_inactive'])) {
            ...
            switch ($pathRecordDetails[0]) {
                ...
                case 'link':
                    if($valueBean && $valueBean->load_relationship($pathRecordDetails[2])){
                        ...
                        LoggerManager::getLogger()->fatal(__CLASS__. '::'.__FUNCTION__.'() Could not load '.$pathRecordDetails[2]. " for {$valueBean->_module}::{$valueBean->id}");
                    }
                    break;
                case 'field':
