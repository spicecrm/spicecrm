...
class SpiceACLProfile extends SpiceBean {
    ...
    public static function getProfilesForUserRows($userId) {
        ...
        $q="SELECT DISTINCT p.id FROM spiceaclprofiles_users pu inner join spiceaclprofiles p on p.id=pu.spiceaclprofile_id where pu.user_id='".$db->quote($userId)."' or pu.user_id='*'";
        if($res = $db->query($q)){
