...
class CampaignTasksController
{
    ...
    {
        $inclusionListId = $this->getAutoGeneratedList($args['id'], 'include');
    ...
    /**
     * get auto generated list for a campaign task and selected type
     * @param string $campaignTaskId
     * @param string $type
     * @return string|null
     * @throws \Exception
     */
    private function getAutoGeneratedList(string $campaignTaskId, string $type): ?string
    {
        return DBManagerFactory::getInstance()->getOne("SELECT pl.id, pl.name FROM prospect_lists pl INNER JOIN prospect_list_campaigntasks plc ON plc.prospect_list_id = pl.id WHERE plc.campaigntask_id = '$campaignTaskId' and pl.is_generated_by_system = 1 and pl.list_type = '$type' and pl.deleted != 1  and plc.deleted != 1 ");
    }
    ...
    public function setTargetsStatus(Request $req, Response $res, array $args): Response
    {
        ...
        $exclusionListId = $this->getAutoGeneratedList($args['id'], 'exclude');

        if (!$exclusionListId && $args['status'] == 'excluded') {
    ...
    public function getCampaignTaskItems(Request $req, Response $res, array $args): Response
    {
        ...
        $list = $campaignLog->get_list(
            "planned_activity_date isnull, planned_activity_date ASC, priority",
