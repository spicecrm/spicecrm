...
class GSuiteBroker {
     ...
     * @param MessageView
     */
    async messageViewHandler(MessageView) {
     ...
     * @return base64
     */
    async downloadEmailAttachment(downloadUrl) {
     ...
     * @param data
     */
    async getEmailMessages(source, data) {
     ...
     */
    async downloadImageToBase64(imageSrc) {
    ...
    async getEmailAddresses() {
        let emailAddresses = [];
        await Promise.all(this.threadView.getMessageViewsAll().map(async (MessageView) => {
            const recipients = await MessageView.getRecipientsFull();
            const sender = await MessageView.getSender().emailAddress;
