...
// config.php
if((file_exists('./config.php') && (!(make_writable('./config.php')) ||  !(is_writable('./config.php')))) || !is_writable('.')) {
...
// config_override.php
if((file_exists('./config_override.php') && (!(make_writable('./config_override.php')) ||  !(is_writable('./config_override.php')))) || !is_writable('.')) {
...
EOT;
        ...
        installLog("Testing web.config redirects");
        if(file_exists("web.config")) {
            $old_htaccess = file_get_contents("web.config");
        }
        $basePath = parse_url($_SESSION['setup_site_url'], PHP_URL_PATH);
        if(empty($basePath)) $basePath = '/';
        $htaccess_test = <<<EOT
...
<configuration>
    <system.webServer>
        ...
        <rewrite>
            <rules>
                <rule name="rule 1v">
                    <match url="^{$basePath}/itest.txt$"  />
                    <action type="Rewrite" url="/{$basePath}/install_test.txt"  appendQueryString="true" />
                </rule>
            </rules>
        </rewrite>
    </system.webServer>
...
EOT;
        ...
        file_put_contents("web.config", $htaccess_test);
        file_put_contents("install_test.txt", "SUCCESS");
        $res = file_get_contents($_SESSION['setup_site_url']."/itest.txt");
        unlink("install_test.txt");
        if(!empty($old_htaccess)) {
            file_put_contents("web.config", $old_htaccess);
        } else {
            unlink("web.config");
        }
        if($res != "SUCCESS") {
          ...
          <tr>
            ...
            installLog("Passed web.config redirects check");
        }
    }
...
////    BEGIN PAGE OUTPUT
  ...
  <table cellspacing="0" cellpadding="0" border="0" align="center" class="shell">
    <tr>
      ...
      {$mod_strings['LBL_CHECKSYS_TITLE']} <br>
    ...
    <tr>
      <td align="right">
        ...
        <table cellspacing="0" cellpadding="0" border="0" class="stdTable">
          <tr>
            <td><input class="button paper-button-0" raised="" role="button" tabindex="0" animated="" aria-disabled="false" elevation="1" type="button" onclick="window.open('http://www.spicecrm.io');" value="{$mod_strings['LBL_HELP']}" /></td>
            <td>
                <input class="button paper-button-0 next" raised="" role="button" tabindex="0" animated="" aria-disabled="false" elevation="1" type="button" name="Re-check" value="{$mod_strings['LBL_CHECKSYS_RECHECK']}" onclick="callSysCheck();" id="button_next2"/>
            </td>
          </tr>
        </table>
        </form>
