...
export class metadata
{
    ...
    loadFieldSets(loadhandler: Subject<string>, forceLoading = false )
    {
        if (
    ...
    loadFieldDefs(loadhandler: Subject<string>, forceLoading = false )
    {
    ...
    loadRoutes(loadhandler: Subject<string>) {
    ...
    loadModuleDefinitions(loadhandler: Subject<string>, forceLoading = false )
    {
        if(
    ...
    * dynamically add routes from this.routes with a route container hat will handle the dynamic routes
     ...
     */
    addComponentDirect(component: string, viewChild: any, injector?: Injector): Observable<any> {
     ...
     */
    addComponent(component: string, viewChild: any, injector?: Injector): Observable<any> {
        let retSubject = new Subject();
                            ...
                            if (this.componentDirectory[component]['module']) {
                                ...
                                System.import(this.moduleDirectory[module].path)
                                    ...
                                    .then((type: any) => {
                                        this.moduleDirectory[module].factories = {};
                                        this.compiler.compileModuleAndAllComponentsAsync(type).then(componentfactory => {
                                            cmp_factory = componentfactory.componentFactories.find((e) => e.componentType.name === component);
                                            if (!cmp_factory) {
     ...
     * getter functions
     */
    getComponentSet(componentSetId) {
        return this.componentSets[componentSetId];
    }

    addComponentSet(id, module, name, type = 'custom') {
    ...
    getComponentSetObjects(componentSetId) {
        try {
    ...
    addComponentToComponentset(id, componentset, item) {
    ...
    getFieldSets(module: string = '', filter: string = '') {
    ...
    addFieldsetToFieldset(id, parent, itemid)
    {
    ...
    addFieldToFieldset(id, parent, field)
    {
        this.fieldSets[parent].items.push({
            id: id,
            field: field,
    ...
    removeFieldsetItem(parent, item) {
        let remIndex = false;
    ...
    getModuleDefs(module) {
        return this.moduleDefs[module];
    }

    getModuleDuplicatecheck(module) {
        try {
    ...
    getModuleFromSingular(singular){
        let module = '';
    ...
    getModuleTrackflag(module): boolean {
        try {
            return parseInt(this.moduleDefs[module].track) ? true : false;
        } catch (e) {
            return false;
        }
    }
    ...
    getModuleValidations(module: string)
    {
        try {
    ...
    getModuleListTypes(module: string) {
        try {
    ...
    addModuleListType(module: string, listTypeData: any) {
        this.moduleDefs[module].listtypes.push(listTypeData);
    }

    getFieldDefs(module: string, field: string) {
        try {
    ...
    checkStatusManaged(module:string){
    ...
    getFieldType(module: string, field: string) {
        try {
    ...
    getFieldOptions(module: string, field: string) {
        try {
    ...
    getFieldRequired(module: string, field: string) {
        try {
     ...
     */
    getComponentConfigOptions(component) {
        try {
     ...
     */
    getComponentConfigurations(module = '*') {
        if (module === '*')
     ...
     */
    getComponentConfig(component: string = '', module: string = '', role = '') {
    ...
    getModuleDefaultComponentConfigByUsage(module:string, usage: 'list'|'details')
    {
        let component = '';
        switch(usage)
        {
    ...
    * check if the module has tagging enabled
    ...
    loadLibs(...scripts:string[]):Observable<object>
    {
        ...
        for(let o of observables)
        {
            o.subscribe(
                ...
                () => {
                    //console.log('completed... '+cnt+' of '+observables.length);
    ...
    getHtmlStylesheetCode( stylesheetId: string ): String {
    ...
    getHtmlFormats( stylesheetId: string ): Array<any> {
        ...
        if ( !_.isArray( this.htmlStyleData.stylesheets[stylesheetId].formats ) ) this.htmlStyleData.stylesheets[stylesheetId].formats = [];
        ...
        if ( !this.htmlStyleData.stylesheets[stylesheetId].stylesDecoded ) this.htmlStylesToObjects( stylesheetId );
        return this.htmlStyleData.stylesheets[stylesheetId].formats;
    }

    htmlStylesToObjects( stylesheetId ) {
        for ( let format of this.htmlStyleData.stylesheets[stylesheetId].formats ) {
            let styles;
            if ( !_.isEmpty( format.styles )) {
                try { styles = JSON.parse( format.styles ); }
                catch( e ) {
                    console.log( 'HTML Styling: Malformed style specification in table sysuihtmlformats (format id: ' + format.id + ').' );
                    styles = {};
                }
                format.styles = styles;
            } else format.styles = null;
        }
        this.htmlStyleData.stylesheets[stylesheetId].stylesDecoded = true;
    }

    getHtmlStylesheetNames(): Array<any> {
        let stylesheets = [];
        for ( let sheetId in this.htmlStyleData.stylesheets ) stylesheets.push ({ id:this.htmlStyleData.stylesheets[sheetId].id, name:this.htmlStyleData.stylesheets[sheetId].name });
        return _.sortBy( stylesheets, 'name');
    }

    getHtmlStylesheetToUse( module: string, fieldname: string ) {
