...
export class modal {
    ...
    */
     ...
     */
    public prompt(type: 'info'|'input'|'input_text'|'input_date'|'confirm'|'input_module'|'input_password', text: string, headertext: string = null, theme: string = 'shade', defaultvalue: string | number = null, options: { value: string|boolean, display: string, disabled?: boolean}[] = null, optionsAs: true|'radio'|'button'|'select' = 'select', regex?: string ): Observable<any> {
    ...
    public await(messagelabel: string = null): EventEmitter<boolean> {
        ...
        this.openStaticModal(SystemLoadingModal, false, this.injector).subscribe(component => {
            component.instance.messagelabel = this.language.getLabel(messagelabel);
            stopper.subscribe(() => {
