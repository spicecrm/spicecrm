...
@Component({
    selector: 'sendgrid-remove-targetlist-from-contact-button',
    templateUrl: '../templates/sendgridremovetargetlistfromcontactbutton.html',

})

export class SendgridRemoveTargetListFromContactButton {
    constructor(
        public model: model,
        ...
        this.modal.confirm('MSG_DELETE_RECORD', 'MSG_DELETE_RECORD')
            .subscribe(answer => {
                if (answer) {
                    ...
                    this.backend.deleteRequest(`channels/emarketing/sendgrid/marketing/lists/${this.model.id}/contacts/${this.relatedmodels.id}/${this.model.data.prospectlists_person_email_addr_bean_rel_id}`).subscribe(
                        response => {
