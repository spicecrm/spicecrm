 ...
 * @module ObjectFields
 */
import {Component, ElementRef, OnDestroy, OnInit, SkipSelf} from '@angular/core';
...
export class fieldLinked extends fieldGeneric implements OnInit, OnDestroy {
    ...
    get value() {
        ...
        return this.fielddefs?.rname && reldata && reldata[this.fielddefs.rname] ? reldata[this.fielddefs.rname] : '';
    }
    ...
    public ngOnInit() {
        super.ngOnInit();
        this.relateIdField = this.fielddefs.id_name;
        this.relateNameField = this.fieldname;
        this.relateType = this.fielddefs.module;
        this.isAuthorized = this.metadata.checkModuleAcl(this.fielddefs.module, 'list') || this.metadata.checkModuleAcl(this.fielddefs.module, 'listrelated');

        // if we do not have a fieldset defined here take the default one
        if(!this.fieldconfig.fieldset){
            let c = this.metadata.getComponentConfig('fieldLinked', this.fielddefs.module);
    ...
    public setRelated(related) {
        ...
        newFields[this.relateNameField] = related.data;
