...
export class ProspectListsSetTargetsEmailAddressModal implements ModalComponentI, OnDestroy {
     ...
     */
    public items: { id: string, summary_text: string; prospectlists_contacts_email_addr_bean_rel_id: string}[] = [];
    /**
     * modal response to pass data to parent
     */
    public response = new Subject<{ id: string; prospectlists_contacts_email_addr_bean_rel_id: string}[]>();
