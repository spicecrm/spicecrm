...
export class DashboardContainerElement implements AfterViewInit {
    ...
    getBoundingBoxStyle(position): any {
    ...
    applyMove(rect){
        let style = rect,
            mainContainer: any = this.dashboardlayout.mainContainer,
            mainContainerRight: number = mainContainer.right - mainContainer.x - this.dashboardlayout.paddingRight,
            margin: number = this.dashboardlayout.boxMargin,
            boxWidth: number = this.dashboardlayout.elementWidth - (2 * margin),
            boxHeight: number = this.dashboardlayout.elementHeight - (2 * margin),
            movex: number,
            movey: number;
        ...
        if (this.mouseStart) {
            switch (this.mouseTarget) {
                ...
                case 'right':
                    ...
                    if (style.width < boxWidth) style.width = boxWidth;
                    break;
                case 'left':
