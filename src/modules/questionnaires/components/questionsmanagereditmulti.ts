...
export class QuestionsManagerEditMulti implements OnInit {
    ...
    public ngOnInit(): void {
        this.model.data$.subscribe(data => {
            if ( data.id && !this.isBuilt ) this.buildEntries(); // model data is already available (loaded) AND buildEntries() has not been executed yet (options are not built yet)
        });
    }
    ...
    private buildEntries(): void {
        ...
        if ( this.model.data.questionparameter && this.model.data.questionparameter !== '' ) {
            ...
            this.hasInfosCorrectness = typeof questionparameter.hasInfosCorrectness != 'undefined' ? questionparameter.hasInfosCorrectness:false;
        }
    }
