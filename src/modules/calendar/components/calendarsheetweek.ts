...
export class CalendarSheetWeek implements OnChanges, AfterViewInit {
    ...
    private getAllMultiEvents() {
        let events = this.ownerMultiEvents.concat(this.otherEvents, this.userMultiEvents, this.googleMultiEvents);
        this.arrangeMultiEvents(events);
        return events;
    }

    private arrangeMultiEvents(events) {
    ...
    private getGoogleEvents() {
        ...
        this.calendar.loadGoogleEvents("Week", this.startDate, this.endDate).subscribe(events => {
            if (events.length > 0) {
                events = this.correctHours(events);
                events = this.filterEvents(events);
    ...
    private getEventStyle(event) {
        ...
        let sheetWidth = this.calendarsheet.element.nativeElement.clientWidth;
    ...
    private getMultiEventStyle(event): any {
        let eventI = null;
        let multiEventsContainerWidth = (this.calendarsheet.element.nativeElement.getBoundingClientRect().width - this.sheetRightMargin - this.sheetTimeWidth) / this.calendar.weekDaysCount;
    ...
    private getDaysContainerStyle() {
        return {
            width: this.calendarsheet.element.nativeElement.clientWidth - this.sheetTimeWidth + 'px'
        };
    }
