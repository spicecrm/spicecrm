...
@Component({
    ...
    templateUrl: '../templates/fieldprojectactivityduration.html',
    standalone: false
})
export class fieldProjectActivityDuration extends fieldGeneric {
    ...
    get minutes() {
        let minutes = Math.floor((this.value % 3600) / 60);
    ...
    set hours(value) {
        if (value) {
            let currentMinutes = parseInt(this.minutes, 10);
            this.value = parseInt(value, 10) * 3600 + currentMinutes * 60;
        }
    }
}
