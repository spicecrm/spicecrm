...
//		}
    ...
    if($dev['type'] == "dev" && !isset($_SESSION['spice_updates'])){
        ...
        if($updates['count'] > 0) {
            $_SESSION['spice_updates'] = $updates['updates'];
            $msg = translate('WARN_NEWSWUPDATES', 'Administration');
            $msg = str_replace('{count}', $updates['count'],$msg);
            $msg = str_replace('{info}', $updates['string'],$msg);
