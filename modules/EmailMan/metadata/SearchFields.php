...
//        'to_email'=> array('query_type'=>'default','db_field'=>array('contacts.email1','leads.email1','prospects.email1')),
        ...
        'to_email'=> array(
			'query_type' => 'default',
			'operator' => 'subquery',
			'subquery' => 'SELECT eabr.bean_id FROM email_addr_bean_rel eabr JOIN email_addresses ea ON (ea.id = eabr.email_address_id) WHERE eabr.deleted=0 AND eabr.bean_module IN(\'Contacts\', \'Leads\', \'Prospects\') AND ea.email_address LIKE',
			'db_field' => array(
				'related_id',
			),
		),    
