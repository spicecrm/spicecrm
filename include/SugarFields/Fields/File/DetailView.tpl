...
<a href="index.php?entryPoint=download&id={$fields.{{$vardef.fileId}}.value}&type={{$vardef.linkModule}}{{if !empty($displayParams.fileid)}}&fileid={$fields.{{$displayParams.fileid}}.value}{{/if}}{{if !empty($displayParams.link)}}&filename={$fields.{{$displayParams.link}}.value|urlencode}{{/if}}" class="tabDetailViewDFLink" target='_blank'>{{sugarvar key='value'}}</a>
</span>
