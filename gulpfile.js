 ...
 */
gulp.task('js:build:subtask:bustcache', async function(done) {
    // settings
    var swigopts = {
        load_json: true,
        // json_path: './data/',
 ...
 * delete dist folder
 */
gulp.task('js:build:subtask:deletedist', async function (done) {
    //delete dist folder synchronously!
    _distSource = ['./dist'];
    await delFiles(_distSource, {force: true});
    done();
});
 ...
 * do the build
 */
gulp.task('js:build:subtask:dobuild', async function(done){
    ...
    await new Promise((resolve, reject) => {
        request({ url: global.reference + '/config/repositorymodules', rejectUnauthorized: false}, async function (error, response, body) {
...
/**
 * compile dist folder ts files
 */
gulp.task('js:dist_compile', function (done) {
...
global.reference = global.customReference ? 'https://'+global.customReference+'.spicecrm.io/proxy/000' : 'https://spicecrmreference.spicecrm.io/proxy/000';

/*
 * runs uglify and uses reserved words from the reference DB
 */
gulp.task('js:uglifyfromreference', async function (done) {

    await new Promise((resolve, reject) => {
        request({url: global.reference + '/config/repositoryitems', rejectUnauthorized: false}, async function (error, response, body) {
...
gulp.task('js:deploy', gulp.series('js:build', 'js:dist_compile', 'js:uglifyfromreference', 'js:build:subtask:deletedist'), function(done) {
    done(); //!important since gulp4
